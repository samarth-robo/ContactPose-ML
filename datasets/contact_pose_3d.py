...
class ContactPose3D(Dataset):
  def __init__(self, data_dir, contactpose_data_dir, grid_size, n_rotations,
               ...
               exclude_sessions=None, train_mode=False):
    super(ContactPose3D, self).__init__()
    setup_logging()
    name = __name__ + ('_train' if train_mode else '_val')
  ...
  def drop_joints_camera(self, oX):
    ...
    # drop_joint_idx = [5, 6, 7, 8]
    mask = np.ones(len(oX)) > 0
    mask[drop_joint_idx] = False
    return mask 
  ...
  def __getitem__(self, index):
    ...
    colors, hand_idx, hand_vectors, hand_dists, joint_dists = \
      np.hsplit(sdata, [1, 2, 5, 6])
    ...
    hand_dots = np.sum(hand_vectors*self.normals[object_name].T, axis=1,
                       keepdims=True)
    hand_dots /= np.linalg.norm(hand_vectors, axis=1, keepdims=True)
    ...
    if filename_idx not in self.filenameidx2keptlines:
      kept_joints = \
          [self.drop_joints_camera(joints[i]) for i in range(len(joints))]
    elif self.eval_mode:
      kept_joints = self.filenameidx2keptlines[filename_idx]
      kept_joints = [(d>0)[np.newaxis, :] for d in kept_joints]
    for i in range(2):
      idx = hand_idx == i
      joint_dists[idx] *= kept_joints[i]

    # sdata = np.hstack((hand_dists, hand_dots, joint_dists))
    sdata = np.hstack((hand_dists, joint_dists))

    out = [occ, sdata.astype(np.float32), ijk.astype(np.float32), colors]
    if self.eval_mode:
      kept_joints = [d.squeeze().astype(np.int) for d in kept_joints]
      self.filenameidx2keptlines[filename_idx] = np.vstack(kept_joints)
...
if __name__ == '__main__':
  ...
  dset = ContactPose3D('data/prediction_data', 'data/contactpose_data', 64, 1,
                       joint_droprate, eval_mode=False,
                       include_sessions=[50],
  ...
  for data in dloader:
    for batch_idx in range(N_show):
      ...
      # dists = data[1][:, 0][data[-2] == batch_idx].numpy().squeeze()
      # dists -= dists.min()
      # colors = 1.0 - (dists / dists.max())
      
      dists = data[1][:, 2+12][data[-2] == batch_idx].numpy().squeeze()
      dists -= dists.min()
      colors = 1.0 - (dists / dists.max())
      
      # dots = data[1][:, 1:2][data[-2] == batch_idx].numpy().squeeze()
      # colors = (dots + 1) / 2
      # colors = np.abs(dots)
