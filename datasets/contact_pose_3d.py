...
class ContactPose3D(Dataset):
  def __init__(self, data_dir, contactdb_data_dir, grid_size, n_rotations,
    ...
    hand_line_ids = get_hand_line_ids()
    self.jointidx2lineidx = {}
    for line_idx, joint_idxs in enumerate(hand_line_ids):
      for joint_idx in joint_idxs:
        if joint_idx not in self.jointidx2lineidx:
          self.jointidx2lineidx[joint_idx] = [line_idx]
        else:
          self.jointidx2lineidx[joint_idx].append(line_idx)
    ...
    for filename in next(os.walk(normals_dir))[-1]:
      ...
      if not osp.isfile(occupancy_filename):
        self.logger.warn('{:s} does not exist'.format(occupancy_filename))
        continue
    ...
    # collect surface data filenames and joint locations
    self.filenames = []
    ...
    for session_name in next(os.walk(data_dir))[1]:
      ...
      for object_name in self.normals.keys():
        ...
        filename = osp.join(contactdb_data_dir, session_name,
  ...
  def drop_lines_random(self, N_joints):
    mask = np.random.rand(1, N_joints)
    mask = mask > self.joint_droprate
    return mask

  def drop_lines_camera(self, oX):
    ...
    drop_line_idx = []
    for joint_idx in drop_joint_idx:
      drop_line_idx.extend(self.jointidx2lineidx[joint_idx])
    mask = np.ones((1, len(oX)-1)) > 0
    mask[0, drop_line_idx] = False
    return mask
  ...
  def __getitem__(self, index):
    ...
    colors, hand_idx, hand_vectors = np.hsplit(sdata, [1, 2])
    colors = np.squeeze(colors)
    hand_idx = np.squeeze(hand_idx)
    ...
    split_idx = np.linspace(
        0, hand_vectors.shape[1], hand_vectors.shape[1]//3+1, dtype=np.int)
    split_idx = split_idx[1:-1]
    hand_dists = np.hstack([np.linalg.norm(v, axis=1, keepdims=True)
                            for v in np.hsplit(hand_vectors, split_idx)])
    hand_dots = np.hstack([np.sum(v*self.normals[object_name].T, axis=1, keepdims=True)
                            for v in np.hsplit(hand_vectors, split_idx)])
    hand_dots /= hand_dists
    # 30 cm TODO take from config
    hand_dists = np.clip(hand_dists, a_max=0.03, a_min=None)

    # drop joints
    # mask = self.drop_lines_random(hand_dists.shape[1])
    joints = self.joint_locations[session_name][object_name]
    if filename_idx not in self.filenameidx2keptlines:
      kept_lines = \
          [self.drop_lines_camera(joints[i]) for i in range(len(joints))]
    elif self.eval_mode:
      kept_lines = self.filenameidx2keptlines[filename_idx]
      kept_lines = [(d>0)[np.newaxis, :] for d in kept_lines]
    for i in range(2):
      idx = hand_idx == i
      hand_dists[idx] *= kept_lines[i]
      hand_dots[idx]  *= kept_lines[i]

    sdata = np.hstack((hand_dists, hand_dots))

    out = [occ, sdata.astype(np.float32), ijk.astype(np.float32), colors]
    if self.eval_mode:
      kept_lines = [d.squeeze().astype(np.int) for d in kept_lines]
      self.filenameidx2keptlines[filename_idx] = np.vstack(kept_lines)
...
def eval_collate_fn(data):
  ...
  for idx, d in enumerate(data):
    ...
    ijks.append(torch.as_tensor(d[2], dtype=torch.float32))
    colors.append(torch.as_tensor(d[3], dtype=torch.long))
    kept_lines.append(torch.as_tensor(d[4], dtype=torch.long))
    batch.append(idx * torch.ones(len(d[1]), dtype=torch.long))
  occs = torch.stack(occs)
  sdatas = torch.cat(sdatas)
  ijks = torch.cat(ijks)
  colors = torch.cat(colors)
  kept_lines = torch.stack(kept_lines)
  batch = torch.cat(batch)
  return occs, sdatas, ijks, batch, colors, kept_lines


if __name__ == '__main__':
  from torch.utils.data import DataLoader
  joint_droprate = 0
  dset = ContactPose3D('data/skeleton_prediction_data', 'data/contactpose_data',
                       64, 1,
                       joint_droprate, eval_mode=False,
                       include_sessions=[35],
                       include_instructions=['handoff'],
                       include_objects=['stapler'])
  ...
  for data in dloader:
    for batch_idx in range(N_show):
      ...
      colors = data[1][:, :20][data[-2] == batch_idx].numpy().squeeze()
      colors[colors == 0] = 5000
      colors = np.min(colors, axis=1)
      colors /= colors.max()
      
      # colors = data[1][:, 20:][data[-2] == batch_idx].numpy().squeeze()
      # dists = data[1][:, :20][data[-2] == batch_idx].numpy().squeeze()
      # colors = colors[np.arange(len(colors)), np.argmin(dists, axis=1)]
      # colors = (colors + 1) / 2
