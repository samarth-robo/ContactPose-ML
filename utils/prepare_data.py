...
def prepare_surface_data(object_name, config, data_dir, binvoxes_dir, output_dir,
  ...
  for this_hand_joints, valid in zip(hand_joints, valid_hands):
    if not valid:
      distances = np.inf * np.ones((len(xyz), len(hand_joints[0])))
    else:
      vectors = this_hand_joints[np.newaxis, :, :] - xyz[:, np.newaxis, :]
      distances = np.linalg.norm(vectors, axis=2)
    hand_joint_distances.append(distances)
  hand_joint_distances = np.hstack(hand_joint_distances)
  hand_idx = np.argmin(hand_joint_distances, axis=1) >= 21

  feats = np.zeros((len(xyz), len(hand_joints[0]), 3))
  for i in range(2):
    idx = hand_idx == i
    if np.sum(idx) == 0:
      continue
    feats[idx, :, :] = hand_joints[i]
  feats = np.reshape(feats, (len(feats), -1))
  feats = np.hstack((xyz, feats))

  data = np.hstack((colors[:, np.newaxis], hand_idx[:, np.newaxis], feats))
  filename = osp.join(
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--output_dir', default=osp.join('data', 'simple-joints_prediction_data'))
  parser.add_argument(
