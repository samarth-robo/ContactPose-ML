...
def train(data_dir, contactpose_dir, split, config_file, experiment_suffix=None,
  ...
  exp_name = '{:s}_split_{:s}_images'.format(model_name, split)
  ...
  if 'images' in model_name:
    ...
    model = ImagePointNet(n_surface_features, len(texture_weights), droprate,
        n_channels, len(used_cameras), n_views)
  ...
  if 'images' in model_name:
    ...
    # the dataset always loads all 3 views
    # model selects the necessary number of views from loaded data
    train_dset = ContactPoseImages(data_dir, contactpose_dir, grid_size,
        n_rotations, use_depth, used_cameras, n_video_frames, im_size,
        n_views=3, rgb_transform=rgb_transform, depth_transform=depth_transform,
        **splits['train'])
    if do_val:
      val_dset = ContactPoseImages(data_dir, contactpose_dir, grid_size,
          n_rotations, use_depth, used_cameras, n_video_frames, im_size,
          n_views=3, rgb_transform=rgb_transform, depth_transform=depth_transform,
          **splits['test'])
  else:
  ...
  train_dloader = DataLoader(train_dset, batch_size=batch_size, shuffle=shuffle,
    pin_memory=True, num_workers=num_workers, collate_fn=dloader_collate_fn)
  if do_val:
